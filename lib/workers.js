...
workers.validate_check_data = (original_check_data)=>{
  ...
  original_check_data.user_phone = typeof(original_check_data.user_phone) == 'string'  && original_check_data.user_phone.trim().length == 10 ? original_check_data.user_phone : false;
...
workers.perform_check = (original_check_data)=>{
  ...
  const outcome_sent = false;

  // Parse the hostname and the path out of the original check data
  const pased_url = url.parse(original_check_data.protocol+'://'+original_check_data.url, true);
